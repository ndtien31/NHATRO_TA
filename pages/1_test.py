...
# --- HÀM FORMAT NGÀY THÁNG ĐÚNG KIỂU KHÔNG DẤU NHÁY ---
            ...
            if not json_files:
                ...
                if not df.empty:
                    ...
                    with st.expander("✏️ Chỉnh sửa dòng đã chọn"):
                        ...
                        if update_btn:
                            try:
                                formatted_row = [clean_month(v) if "THANG" in k else clean_date(v) if "Ngày" in k else str(v).strip() for k, v in edited_row.items()]
                                col_count = len(formatted_row)
                                end_col_letter = colnum_to_excel_col(col_count)
                                sheet.update(f"A{selected_row+2}:{end_col_letter}{selected_row+2}", [formatted_row])
                    ...
                    with st.expander("➕ Thêm dòng mới từ dòng đã chọn"):
                        with st.form("duplicate_form"):
                            ...
                            if st.form_submit_button("📥 Thêm dòng mới"):
                                try:
                                    formatted_row = [clean_month(v) if "THANG" in k else clean_date(v) if "Ngày" in k else str(v).strip() for k, v in new_row.items()]
                        ...
                        with st.form("nhap_chiso_moi"):
                            ...
                            if capnhat_btn:
                                try:
                                    ...
                                    row_values = [clean_month(v) if "THANG" in k else clean_date(v) if "Ngày" in k else str(v).strip() for k, v in df.loc[idx].items()]
                                    col_count = len(row_values)
                                    end_col_letter = colnum_to_excel_col(col_count)
                                    sheet.update(f"A{idx+2}:{end_col_letter}{idx+2}", [row_values])
                        ...
                        try:
                            ...
                            for _, row in latest_rows.iterrows():
                                ...
                                for col in ['Chỉ số điện mới', 'Chỉ số nước mới', 'Đã thanh toán', 'Ngày thanh toán', 'Số tiền đã trả', 'Hình thức thanh toán', 'Tiền điện', 'Tiền nước', 'Tổng cộng']:
                                    if col in new_row:
                                        new_row[col] = ""
                                row_values = [clean_month(v) if "THANG" in k else clean_date(v) if "Ngày" in k else str(v).strip() for k, v in new_row[df.columns].items()]

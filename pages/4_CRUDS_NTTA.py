    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            ...
            if not json_files:
                ...
                def load_data():
                    try:
                        sheet = client.open_by_url(sheet_url).sheet1  # Hoặc worksheet("DULIEU_FULL") nếu có
                ...
                df, sheet = load_data()
                if not df.empty:
                    selected_row = st.selectbox("📝 Chọn dòng để chỉnh sửa hoặc xoá:", df.index, format_func=lambda i: f"Phòng: {df.iloc[i]['Số phòng']} - Khách: {df.iloc[i]['Họ tên khách thuê']}")
